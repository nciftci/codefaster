...
switch( $all_url_vars[ 'do' ] ) {
     ...
     **/
        ...
        $Obj = new Listing();
        $Obj -> init_mysql( DB_PREFIX . 'product', array( id, name ) );// set the LIMIT on page, default is 15
        $limit = 15;//the number of page in the pagination bar
        $max = 4;
        $gpos = ( isset( $_GET [ 'page' ] ) ? $_GET[ 'page' ]/$limit : 0 );
        $Obj -> setLimit( $limit );
        $field = array( '{LANG_ADMIN_TESTPRODUCT_ID}', '{LANG_ADMIN_TESTPRODUCT_NAME}', );
        $Obj -> setFields( $field );
     ...
     **/
        ...
        $ft -> assign( 'ACTIVE', $_SESSION[ 'active' ] ); {
            ...
            //$ft -> assign( 'BROWSE_FILEUPLOAD', $util -> getUploadImagesHtml( $testproduct -> getfileupload() ) );
            $labels = array();
            array_push($labels, LANG_ADMIN_CATEGORY_IMAGE);
    ...
    // TODO: We need here to give back the errors. You can customize it. Ex: check if email, via a function (class).
        ...
        if( !empty( $err ) ) {
            ...
            if( empty( $all_url_vars[ 'id' ] ) ) {// value is empty, new items will be INSERTed
                $testproduct = new TestProduct( 0, $LANG );
            }
            else {// value have ID, all data will be UDPATEd
                $testproduct = new TestProduct( $all_url_vars[ 'id' ], $LANG );
            }
            ...
            $upload_image_check=!empty($all_url_vars["check"]);

            if (!$upload_image_check) $testproduct -> setfileupload( $util -> uploadAllImages( 'TestProduct' ,FU_CONF_UPLOADDIR) );
