...
**/
	...
	//};
	$Obj -> init_mysql( DB_PREFIX . 'product', array( name, descriptionshort_en, description_en ) );// replace data from listing from other table
	...
	$Obj -> setLimit( $limit );
	$field = array( '{LANG_ADMIN_TESTPRODUCT_NAME}', '{LANG_ADMIN_TESTPRODUCT_DESCRIPTIONSHORT_EN}', '{LANG_ADMIN_TESTPRODUCT_DESCRIPTION_EN}', );
	$Obj -> setFields( $field );
	$offset = $Obj -> getOffset( ( $gpos+$limit )/$limit );//set link css
	$Obj -> setFirstID( 'id' );
	$Obj -> setStyle( 'gotopage' );//enable active column on listing
	$Obj -> setActivateListing( 0 );//do not remove
	$ft -> assign( 'LISTING', '1' );
...
**/
	...
	$ft -> assign( 'DESCRIPTION_EN', $_SESSION[ 'description_en' ] );
	...
	$ft -> assign( 'ACTIVE', $_SESSION[ 'active' ] );
	{
		...
		$ft -> assign( 'DESCRIPTION_EN', htmlspecialchars_decode( $testproduct -> getdescription_en() ) );
...
**/
	...
	if( !empty( $err ) )
	{
		...
		$_SESSION[ 'description_en' ] = $all_url_vars[ 'description_en' ];
	...
	else
	{
		...
		$testproduct -> setdescriptionshort_en( htmlspecialchars( $all_url_vars[ 'descriptionshort_en' ] ) );
		$testproduct -> setdescription_en( htmlspecialchars( $all_url_vars[ 'description_en' ] ) );
