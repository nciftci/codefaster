...
**/
	...
	$Obj = new Listing();
	$Obj -> init_mysql( DB_PREFIX . 'product', array( id, name, categoryid, fileupload, active ) );// replace data from listing from other table
...
**/
	...
	$ft -> assign( 'ACTIVE', $_SESSION[ 'active' ] );
	{
		...
		$ft -> assign( 'NAME', htmlspecialchars_decode( $testproduct -> getname() ) );
		$ft -> assign( 'DESCRIPTIONSHORT', htmlspecialchars_decode( $testproduct -> getdescriptionshort() ) );
		...
		$ft -> assign( 'SELECTEDRD_ISTHISREAL_1',( $testproduct -> getisthisreal() == 1?'checked':'' ) );
		$ft -> assign( 'SELECTEDRD_ISTHISREAL_0',( $testproduct -> getisthisreal() == 1?'':'checked' ) );
...
**/
	...
	if( empty( $all_url_vars[ 'categoryid' ] ) )
	{
		$err = LANG_ADMIN_TESTPRODUCT_VERIF_CATEGORYID;
	}
	
	if( empty( $all_url_vars[ 'isthisreal' ] ) )
	{
		$err = LANG_ADMIN_TESTPRODUCT_VERIF_ISTHISREAL;
	}
	
	if( empty( $all_url_vars[ 'termsagree' ] ) )
	{
		$err = LANG_ADMIN_TESTPRODUCT_VERIF_TERMSAGREE;
	}
	...
	else
	{
		...
		$testproduct -> setname( htmlspecialchars( $all_url_vars[ 'name' ] ) );
		$testproduct -> setdescriptionshort( htmlspecialchars( $all_url_vars[ 'descriptionshort' ] ) );
		...
		if( !empty( $all_url_vars[ 'fileupload_delete' ] ) )
		{
		...
		if( !empty( $cropping_files_array ) )
		{
			...
			;
			$_SESSION[ 'CROP_INFO' ] = $all_crop_info;//	$_SESSION['id'] = $all_url_vars[ 'id' ];
			$_SESSION[ 'id' ] = $testproduct -> getId();//Kszili Crop invalid if $all_url_vars is used
