...
**/
	public function __construct( $id = 0, $lang = 'en' )
	{
		...
		$SQL = " SELECT `name`,`descriptionshort_$this->lang`,`description_$this->lang`,`categoryid`,`isthisreal`,`termsagree`,`fileupload`,`active` FROM `" . DB_PREFIX . 'product` ' . $this -> where_clause;
		...
		if( $row = mysql_fetch_array( $retid ) )
		{
			...
			$this -> description_en = $row[ "description_$this->lang" ];
...
* @author   - Test Developer
* @type     - public
* @desc     - set $this->description_en to $description_en
* @param    - string $description_en
* @return   - void
* @vers     - 1.0
* @Mod  	-
**/
	public function setDescription( $description_en )
	{
...
**/
	public function save()
	{
		...
		{
			$SQL = 'INSERT INTO `' . DB_PREFIX . "product` (`name`,`descriptionshort_$this->lang`,`description_$this->lang`,`categoryid`,`isthisreal`,`termsagree`,`fileupload`,`active`)";
			$SQL .= ' VALUES(\'' . $this -> slashes( $this -> name ) . '\',\'' . $this -> slashes( $this -> descriptionshort_en ) . '\',\'' . $this -> slashes( $this -> description_en ) . '\',\'' . $this -> slashes( $this -> categoryid ) . '\',\'' . $this -> slashes( $this -> isthisreal ) . '\',\'' . $this -> slashes( $this -> termsagree ) . '\',\'' . $this -> slashes( $this -> fileupload ) . '\',\'' . $this -> slashes( $this -> active ) . '\')';
		...
		else
		{
			...
			$SQL = 'UPDATE `' . DB_PREFIX . 'product` SET `name`=\'' . $this -> slashes( $this -> name ) . "',`descriptionshort_$this->lang`='" . $this -> slashes( $this -> descriptionshort_en ) . "',`description_$this->lang`='" . $this -> slashes( $this -> description_en ) . '\',`categoryid`=\'' . $this -> slashes( $this -> categoryid ) . '\',`isthisreal`=\'' . $this -> slashes( $this -> isthisreal ) . '\',`termsagree`=\'' . $this -> slashes( $this -> termsagree ) . '\',`fileupload`=\'' . $this -> slashes( $this -> fileupload ) . '\',`active`=\'' . $this -> slashes( $this -> active ) . '\'';
