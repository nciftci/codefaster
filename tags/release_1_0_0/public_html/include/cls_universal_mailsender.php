...
*/
    ...
    */
    function SendMailLinux()
    {
        ...
        if (!empty($this->mail_sender) && version_compare(phpversion(), "4.0", ">=")) {
            $temp_from = ini_get("sendmail_from");
            ini_set("sendmail_from", $this->mail_sender);
        }

        if (!empty($this->mail_sender) && version_compare(phpversion(), "4.3.0", ">=")) {
            $extra_headers = "-oi -f " . $this->mail_sender;
            $res = @mail($this->clean($this->mail_to), $this->mail_subject, $this->mail_body, $this->mail_headers, $extra_headers);
        } else

            $res = @mail($this->clean($this->mail_to_addr), $this->mail_subject, $this->mail_body, $this->mail_header);
